...
export default class Input {
  ...
  public setDAS(ms: number) {
    this.DAS = Math.max(0, Math.floor(ms));
  }
  public getARR() {
    return this.ARR;
  }
  public setARR(ms: number) {
    this.ARR = Math.max(0, Math.floor(ms));
  }
  public getSoftDropInterval() {
    return this.SOFT_DROP_INTERVAL;
  }
  public setSoftDropInterval(ms: number) {
    this.SOFT_DROP_INTERVAL = Math.max(0, Math.floor(ms));
  }
  ...
  public reset() {
    try {
      if (this.leftDasTimer) {
        (globalThis as any).clearTimeout(this.leftDasTimer);
        this.leftDasTimer = null;
      }
    } catch (e) {}
    try {
      if (this.leftArrTimer) {
        (globalThis as any).clearInterval(this.leftArrTimer);
        this.leftArrTimer = null;
      }
    } catch (e) {}
    try {
      if (this.rightDasTimer) {
        (globalThis as any).clearTimeout(this.rightDasTimer);
        this.rightDasTimer = null;
      }
    } catch (e) {}
    try {
      if (this.rightArrTimer) {
        (globalThis as any).clearInterval(this.rightArrTimer);
        this.rightArrTimer = null;
      }
    } catch (e) {}
    try {
      if (this.downTimer) {
        (globalThis as any).clearInterval(this.downTimer);
        this.downTimer = null;
      }
    } catch (e) {}
    this.keyState = {};
  ...
  private onKeyDown(e: KeyboardEvent) {
    ...
    try {
      ...
      if (
        ai &&
        typeof ai.isEnabled === "function" &&
        ai.isEnabled() &&
        typeof ai.getDisableInputDuringRun === "function" &&
        ai.getDisableInputDuringRun()
      ) {
    ...
    switch (k) {
      case "ArrowLeft":
        ...
        try {
          ...
          this.eventLog.push({
            ...
            key: k,
            time: this.now(),
            pos: s.current ? { x: s.current.x, y: s.current.y } : null,
          });
        } catch (e) {}
          ...
          this.leftArrTimer = (globalThis as any).setInterval(() => {
            ...
            try {
              ...
              this.eventLog.push({
                ...
                key: k,
                time: this.now(),
                pos: s2.current ? { x: s2.current.x, y: s2.current.y } : null,
              });
            } catch (e) {}
          }, this.ARR);
        }, this.DAS);
      ...
      case "C":
        try {
          if (typeof this.game.holdPiece === "function") this.game.holdPiece(1);
        } catch (e) {}
        break;
      case "v":
      case "V":
        try {
          if (typeof this.game.holdPiece === "function") this.game.holdPiece(2);
        } catch (e) {}
        break;
      case "p":
      case "P":
