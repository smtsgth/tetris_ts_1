...
export default class Renderer {
  ...
  private resizeNextCanvas() {
    ...
    try {
      if (nextWrapper) {
        ...
        const labelEl = nextWrapper.querySelector(
          ".label",
        ) as HTMLElement | null;
        if (labelEl) {
    ...
    const innerHeight = Math.max(
      0,
      boardHeight - padTop - padBottom - labelHeight - labelMarginBottom,
    );
    ...
    try {
      if (this.boardCanvas) {
        fitCanvasPx(
          this.boardCanvas,
          COLS * this.nextCellSize,
          VISIBLE_ROWS * this.nextCellSize,
        );
      }
    } catch (e) {}
    // recompute final board height and next inner height after board resize
    try {
      const finalBoardHeight = this.boardCanvas
        ? this.boardCanvas.offsetHeight
        : boardHeight;
      if (nextWrapper) {
        nextWrapper.style.height = `${finalBoardHeight}px`;
        nextWrapper.style.boxSizing = "border-box";
      }
      const finalInnerHeight = Math.max(
        0,
        (this.boardCanvas ? this.boardCanvas.offsetHeight : boardHeight) -
          padTop -
          padBottom -
          labelHeight -
          labelMarginBottom,
      ...
      fitCanvasPx(
        this.nextCanvas,
        4 * this.nextCellSize + sidePadding * 2,
        finalInnerHeight,
      );
    } catch (e) {}
  }
  ...
  _initDOM() {
    try {
      ...
      this.holdCanvas1 = document.getElementById(
        "hold-canvas-1",
      ) as HTMLCanvasElement | null;
      this.holdCanvas2 = document.getElementById(
        "hold-canvas-2",
      ) as HTMLCanvasElement | null;
      ...
      if (this.overlayEl && this.boardCanvas) {
        ...
        const bh =
          this.boardCanvas.style.height || `${VISIBLE_ROWS * BLOCK_SIZE}px`;
      ...
      this.pauseResumeBtn = document.getElementById(
        "pause-resume-btn",
      ) as HTMLButtonElement | null;
      this.pauseRestartBtn = document.getElementById(
        "pause-restart-btn",
      ) as HTMLButtonElement | null;
  ...
  updateOverlayPositions() {
    ...
    const parentRect =
      this.overlayEl && (this.overlayEl.offsetParent as Element)
        ? (this.overlayEl.offsetParent as Element).getBoundingClientRect()
    ...
    if (this.gameOverEl) {
      ...
      this.gameOverEl.style.display = this.game.getState().over
        ? "flex"
        : "none";
    ...
    if (this.pauseOverlayEl) {
      ...
      try {
        ...
        if (s.paused && !s.over) {
          ...
          try {
            if (!this.game.getState().over)
              this.boardCanvas?.classList.remove("dimmed");
          } catch (e) {}
        }
      } catch (e) {}
    }
  ...
  render() {
    ...
    const boardCtx = this.boardCanvas
      ? this.boardCanvas.getContext("2d")
      : null;
    const nextCtx = this.nextCanvas ? this.nextCanvas.getContext("2d") : null;
    const holdCtx1 = this.holdCanvas1
      ? this.holdCanvas1.getContext("2d")
      : null;
    const holdCtx2 = this.holdCanvas2
      ? this.holdCanvas2.getContext("2d")
      : null;
    ...
    if (boardCtx && cur) {
      ...
      try {
        ...
        if (state.score > this.prevScore) {
          ...
          this.scoreAnimFrom =
            typeof this.displayScore === "number"
              ? this.displayScore
              : this.prevScore;
    ...
    try {
      const now2 = Date.now();
      if (
        this.lineFlashExpiry &&
        now2 < this.lineFlashExpiry &&
        boardCtx &&
        this.boardCanvas
      ) {
        ...
        boardCtx.fillRect(
          0,
          0,
          this.boardCanvas.clientWidth,
          this.boardCanvas.clientHeight,
        );
      }
    } catch (e) {}
        ...
        for (let r = 0; r < mat2.length; r++) {
          for (let c = 0; c < mat2[r].length; c++) {
            ...
            holdCtx2.fillRect(
              x + borderCell2,
              y + borderCell2,
              Math.max(0, cell2 - borderCell2 * 2),
              Math.max(0, cell2 - borderCell2 * 2),
            );
          }
        }
    ...
    try {
      ...
      if (this.scoreAnimStart && this.scoreAnimStart > 0) {
        const t = Math.min(
          1,
          (nowAnim2 - this.scoreAnimStart) / this.scoreAnimDuration,
        );
        const eased = 1 - Math.pow(1 - t, 3); // easeOutCubic
        this.displayScore = Math.floor(
          this.scoreAnimFrom + (this.scoreAnimTo - this.scoreAnimFrom) * eased,
        );
      ...
      try {
        if (
          ...
          if (last && last.startsWith("Hold1") && this.holdBox1El) {
            ...
            this.holdFlashTimer1 = window.setTimeout(() => {
              try {
                if (this.holdBox1El) {
                  this.holdBox1El.classList.remove("hold-flash-saved");
                  this.holdBox1El.classList.remove("hold-flash-swapped");
          ...
          if (last && last.startsWith("Hold2") && this.holdBox2El) {
            ...
            this.holdFlashTimer2 = window.setTimeout(() => {
              try {
                if (this.holdBox2El) {
                  this.holdBox2El.classList.remove("hold-flash-saved");
                  this.holdBox2El.classList.remove("hold-flash-swapped");
      ...
      if (a11y) {
        const last =
          state.notifications && state.notifications.length
            ? state.notifications[state.notifications.length - 1].msg
            : null;
        if (last) {
