...
const token =
  process.env.GITHUB_TOKEN || process.env.GH_TOKEN || process.env.GIT_TOKEN;
if (!token) {
...
(async () => {
  ...
  for (const pr of prs) {
    ...
    console.log(
      `Processing PR #${num}: ${title} (head: ${headRef} from ${headRepoFull})`,
    );
    try {
    ...
    if (
      ...
      try {
        const delRes = await request(
          "DELETE",
          `/repos/${owner}/${repo}/git/refs/heads/${encodeURIComponent(headRef)}`,
        );
        console.log(
          `Deleted remote branch ${headRef} (status ${delRes.statusCode})`,
        );
      } catch (e) {
        console.error(
          `Failed to delete remote branch ${headRef}:`,
          (e && e.message) || e,
        );
      }
    } else {
      console.log(
        `Skipping branch deletion for ${headRef} (not same repo or excluded).`,
      );
    }
  }
})().catch((e) => {
