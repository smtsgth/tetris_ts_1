...
export default class Renderer {
    ...
    resizeNextCanvas() {
        ...
        // derive board cell size (CSS pixels per block)
        let boardCellSize = BLOCK_SIZE;
        try {
            if (this.boardCanvas) {
                const boardWidth = this.boardCanvas.clientWidth || parseFloat(this.boardCanvas.style.width) || (COLS * BLOCK_SIZE);
                boardCellSize = boardWidth / COLS;
            }
        }
        catch (e) { }
        if (!(boardCellSize > 0))
            boardCellSize = BLOCK_SIZE;

        // compute a cell size that allows the configured nextCount to fit vertically
        // shrink previews if necessary so all configured previews fit into the available innerHeight
        const fitSize = innerHeight / (this.nextCount * 4);
        const cellSize = Math.max(4, Math.min(boardCellSize, fitSize));
        this.nextCellSize = cellSize > 0 ? cellSize : BLOCK_SIZE;
        // prefer showing the configured nextCount (we shrank previews to fit)
        this.nextCountUsed = this.nextCount;
        // add horizontal side padding (keep a small minimum — further reduced)
        ...
        try {
            if (this.holdCanvas1)
                fitCanvasPx(this.holdCanvas1, 4 * this.nextCellSize, 4 * this.nextCellSize);
            if (this.holdCanvas2)
                fitCanvasPx(this.holdCanvas2, 4 * this.nextCellSize, 4 * this.nextCellSize);
        }
        catch (e) { }
        // ensure the wrapper's width matches 4 blocks + padding and prevent flex shrink
        try {
            if (nextWrapper) {
                const w = Math.max(0, Math.round(4 * this.nextCellSize + sidePadding * 2));
                nextWrapper.style.minWidth = `${w}px`;
                nextWrapper.style.width = `${w}px`;
                nextWrapper.style.boxSizing = 'border-box';
                try { nextWrapper.style.flex = '0 0 auto'; }
                catch (e) { }
            }
        }
        catch (e) { }
    ...
    updateOverlayPositions() {
        ...
        const parentRect = (this.overlayEl && this.overlayEl.offsetParent) ? this.overlayEl.offsetParent.getBoundingClientRect() : { left: 0, top: 0 };
    ...
    render() {
        ...
        if (nextCtx && this.nextCanvas) {
            ...
            const usedNextCount = (this.nextCountUsed && this.nextCountUsed > 0) ? this.nextCountUsed : this.nextCount;
            const next = (state.next || []).slice(0, usedNextCount);
            const cell = this.nextCellSize;
            // tighter inner gap in Next preview
            const border = Math.max(0.5, cell * 0.035);
            // pack previews: use tighter stacking (less gap between previews)
            const gapBetweenPreviewsBlocks = 1.0;
            const slotBlocks = 4 - gapBetweenPreviewsBlocks;
            // compute total content height using actual previews to be drawn
            const contentHeight = cell * (usedNextCount * slotBlocks);
