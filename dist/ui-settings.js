...
export default function attachInputSettings(input) {
    ...
    onReady(() => {
        ...
        try {
            const aiLabelEls = Array.from(document.querySelectorAll("#ai-panel .setting-row label, #right-controls #ai-panel .setting-row label"));
        ...
        try {
            ...
            if (nextRange && nextNum && renderer && game) {
                const cur = typeof renderer.getNextCount === "function"
                    ...
                    if (lockExportCsvBtn)
                        lockExportCsvBtn.addEventListener("click", () => {
                            ...
                            try {
                                ...
                                const ev2 = typeof game.getLockEvents === "function"
                                    ? game.getLockEvents()
                                    : [];
                                if (!ev2 || !ev2.length)
                                    return;
                    ...
                    function lockRefresh() {
                        try {
                            if (!lockStatsEl || !lockEventsList)
                                return;
                            const s = typeof game.getLockStats === "function"
                                ? game.getLockStats()
                                : {};
                            const ev = typeof game.getLockEvents === "function"
                                ? game.getLockEvents()
                                : [];
