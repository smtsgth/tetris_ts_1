...
const re = /^tuned_250ms_pool4_prewarm_fixedenv_run(\d+)-report\.json$/;
...
const means = files.map(f=>{
  try { const j = JSON.parse(fs.readFileSync(path+'/'+f,'utf8')); return Number(j.meanOfMeans || j.mean || j.meanOfMeans); } catch(e) { return null; }
}).filter(v=>v!=null);
...
fs.writeFileSync(path+'/tuned_250ms_pool4_prewarm_fixedenv-all-report.json', JSON.stringify(out, null, 2));
console.log('wrote', path+'/tuned_250ms_pool4_prewarm_fixedenv-all-report.json');
