...
const re = /^tuned_250ms_pool4_probeBudget(\d+)_run(\d+)-report\.json$/;
...
for (const f of files) {
  ...
  if (!m) continue;
  const budget = m[1];
  if (!groups[budget]) groups[budget] = [];
  groups[budget].push(f);
}
for (const budget of Object.keys(groups).sort(
  (a, b) => parseInt(a) - parseInt(b),
)) {
  const fns = groups[budget].sort((a, b) => {
  ...
  fs.writeFileSync(
    path + `/tuned_250ms_pool4_probeBudget${budget}-fixedenv-all-report.json`,
