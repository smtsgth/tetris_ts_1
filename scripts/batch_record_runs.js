...
for (let i = 1; i <= runs; i++) {
  ...
  const res = cp.spawnSync('node', ['scripts/record_das_timestamps.js'], { stdio: 'inherit', shell: true });
  if (res.error || res.status !== 0) {
    console.error('record_das_timestamps.js failed', res.error || ('exit ' + res.status));
    process.exit(2);
  }
  ...
  const imgs = [`left_ts_fast.png`,`right_ts_fast.png`,`down_ts_fast.png`,`left_ts_slow.png`,`right_ts_slow.png`,`down_ts_slow.png`];
