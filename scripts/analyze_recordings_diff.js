...
const REC_DIR = process.env.REC_DIR
  ? path.resolve(process.env.REC_DIR)
...
function tsFrom(name) {
  const m = name.match(/_(\d{10,})/);
  return m ? Number(m[1]) : null;
}
function readJson(name) {
  try {
    return JSON.parse(fs.readFileSync(path.join(REC_DIR, name), "utf8"));
  } catch (e) {
    return null;
  }
}
function readText(name) {
  try {
    return fs.readFileSync(path.join(REC_DIR, name), "utf8");
  } catch (e) {
    return null;
  }
}
...
const opts = {
  ...
  excerptLines: 50,
  windowMs: 5 * 60 * 1000,
  outPrefix: "diff_report",
};
for (const a of rawArgs) {
  if (a === "--no-csv") opts.csv = false;
  else if (a === "--csv") opts.csv = true;
  else if (a.startsWith("--excerpt="))
    opts.excerptLines = Number(a.split("=")[1]) || opts.excerptLines;
  else if (a.startsWith("--window="))
    opts.windowMs = Number(a.split("=")[1]) || opts.windowMs;
  else if (a.startsWith("--prefix="))
    opts.outPrefix = a.split("=")[1] || opts.outPrefix;
}

const files = listFiles();
const summaryFiles = files.filter((f) =>
  /^auto_test_summary_\d+\.json$/.test(f),
);
const runEventsFiles = files.filter((f) =>
  /^auto_test_run_events_\d+\.json$/.test(f),
);
const fatalFiles = files.filter((f) => /^run_fatal_\d+\.log$/.test(f));
const uncaughtFiles = files.filter((f) =>
  /^run_uncaught_exception_\d+\.log$/.test(f),
);
...
const runs = summaryFiles.map((summary) => {
  ...
  const fatalNearby = fatalFiles.filter(
    (f) => Math.abs((tsFrom(f) || 0) - summaryTs) < opts.windowMs,
  );
  const uncaughtNearby = uncaughtFiles.filter(
    (f) => Math.abs((tsFrom(f) || 0) - summaryTs) < opts.windowMs,
  );
  const serverNearby = serverFiles.filter(
    (f) => Math.abs((tsFrom(f) || 0) - summaryTs) < opts.windowMs,
  );
  ...
  return {
    ...
    hasFatal:
      hasFatalEvent || fatalNearby.length > 0 || uncaughtNearby.length > 0,
...
function diffCountsMap(A, B) {
  ...
  for (const k of keys) {
    ...
    for (const metric of metrics) {
      const an =
        typeof a[metric] === "number" ? a[metric] : Number(a[metric] || 0);
      const bn =
        typeof b[metric] === "number" ? b[metric] : Number(b[metric] || 0);
      m[metric] = an - bn;
    }
    out[k] = m;
  }
...
let prevOk = null;
if (failed) {
  const prevs = runs.filter(
    (r) => !r.hasFatal && r.summaryTs < failed.summaryTs,
  );
...
const report = {
  ...
  runs: runs.map((r) => ({
    summaryFile: r.summaryFile,
    summaryTs: r.summaryTs,
    hasFatal: r.hasFatal,
    eventsFile: r.eventsFile,
    serverFiles: r.serverFiles,
    fatalFiles: r.fatalFiles,
  })),
};
...
if (failed && prevOk) {
  report.comparison = {
    ...
    previousOk: {
      summaryFile: prevOk.summaryFile,
      summaryTs: prevOk.summaryTs,
    },
  ...
  report.note =
    "failed run(s) present but no previous successful run found to compare against.";
}
...
const outFatalExcerptsPath = path.join(
  REC_DIR,
  `${opts.outPrefix}_${Date.now()}_fatal_excerpts.txt`,
);
...
if (report.comparison) {
  ...
  if (
    report.comparison.failedFatalFiles &&
    report.comparison.failedFatalFiles.length
  ) {
    txt += `\nFailed fatal logs:\n`;
...
if (opts.csv) {
  const header = [
    ...
    "hasFatal",
    "totalEarlyFallback",
    "totalFallbackApplied",
    "totalWorkerOverwrite",
    "totalWorkerIgnored",
    "totalTimeout",
    "fatalFiles",
    "uncaughtFiles",
...
console.log(
  ...
  outTxtPath,
  opts.csv ? outCsvPath : "",
  fs.existsSync(outFatalExcerptsPath) ? outFatalExcerptsPath : "",
);
process.exit(0);
