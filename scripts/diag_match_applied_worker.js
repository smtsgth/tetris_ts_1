...
for (const f of files) {
  ...
  for (const line of sample) {
    ...
    if (ev.indexOf("fallback-applied") !== -1) {
      ...
      if (mm) {
        reqMap[id].fallbackSig = mm[1];
        reqMap[id].fallbackScore = Number(mm[2]);
      }
    }
    if (
      ev.indexOf("worker-intermediate") !== -1 ||
      ev.indexOf("worker-intermediate sig:") !== -1
    ) {
  ...
  for (const id of Object.keys(reqMap)) {
    ...
    out.push({
      file: f,
      reqId: id,
      hadFallback: !!r.fallbackSig,
      fallbackScore: r.fallbackScore,
      rawMsgsCount: r.rawMsgs.length,
      workerIntermediates: r.workerIntermediates.length,
      workerResults: r.workerResults.length,
      events: r.events,
    });
  }
}
fs.writeFileSync(
  path.join(dir, "diag_applied_worker.json"),
