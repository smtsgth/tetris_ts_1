...
const penalties = process.argv[2] ? process.argv[2].split(',').map(x=>Number(x)) : [80,160,320,640];
...
const params = { DEBOUNCE_MS: 200, DEBOUNCE_HOLD_MS: 800, MAX_INTERMEDIATE_APPLIES_PER_REQ: 1 };
...
function replaceHoldPenalty(content, newVal){
  return content.replace(/HOLD_PENALTY\s*=\s*\d+\s*;/, `HOLD_PENALTY = ${newVal};`);
}
...
(async function(){
  ...
  const summary = { timestamp: Date.now(), duration, repeats, params, penalties: [] };
  try{
    for(const pen of penalties){
      ...
      const totals = runs.reduce((acc,r)=>{
        if(r && r.applyCount!=null){ acc.apply += r.applyCount; acc.hold += r.holdCount; acc.skipped += r.skipDup; acc.success +=1; } else acc.fail +=1; return acc;
  ...
  const outPath = path.join(recordingsDir, `worker_penalty_sweep_${Date.now()}.json`);
