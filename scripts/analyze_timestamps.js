...
function analyzeKeyEvents(events) {
  ...
  return {
    ...
    medianARR_ms:
      median(intervals) === null ? null : +median(intervals).toFixed(3),
...
function main() {
  const inputs = [
    "recordings/timestamps_fast.json",
    "recordings/timestamps_slow.json",
  ];
  const out = {};
  ...
  fs.writeFileSync(
    "recordings/analysis_timestamps.json",
