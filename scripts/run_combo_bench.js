...
(async function main(){
  try {
    ...
    const outFile = out || path.join(benchDir, `combo_bench_full_auto_${ts}.json`);
    ...
    try {
      await page.waitForFunction(() => window.ai && typeof window.ai.probeStateWithParams === 'function', { timeout: 15000 });
      console.log('Detected window.ai');
    } catch (e) {
      console.warn('window.ai not detected within timeout; continuing');
    }

    const res = await page.evaluate(async (trials, method, budgetMs) => {
      const ai = window.ai || {};
      const pools = [1,2,3,4];
      ...
      for (const pool of pools) {
        ...
        for (const timeout of timeouts) {
          for (const perNodeLimit of perNodeLimits) {
            try { if (typeof ai.setPerNodeLimit === 'function') ai.setPerNodeLimit(perNodeLimit); } catch(e){}
            for (const topK of topKs) {
              for (const beamWidthBase of beamWidthBases) {
                ...
                for (let t = 0; t < trials; t++) {
                  ...
                  try {
                    if (method === 'decideWithBudget' && typeof ai.decideWithBudget === 'function') {
                      const b = (typeof budgetMs === 'number' && !isNaN(budgetMs)) ? budgetMs : timeout;
                      await ai.decideWithBudget(b, { perNodeLimit, beamWidthBase, topK });
                    } else if (typeof ai.probeStateWithParams === 'function') {
                      await ai.probeStateWithParams(state, {}, { beamWidthBase, perNodeLimit, topK }, timeout);
                    } else if (typeof ai.runParameterSweep === 'function') {
                      await ai.runParameterSweep({ beamWidthBase, perNodeLimit, topK, trials: 1 }, timeout);
                    } else {
                ...
                results.push({ pool, timeout, perNodeLimit, topK, beamWidthBase, trials, sampleTimes });
      ...
      const out = { startedAt, endedAt, durationMs: endedAt - startedAt, results };
      window.__combo_bench_results = out;
      window.__lastComboBenchResults = out.results;
      return out;
    }, trials, method, budgetMs);
