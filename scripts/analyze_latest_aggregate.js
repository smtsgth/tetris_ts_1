...
const files = fs.readdirSync(REC).filter(f=>f.startsWith('profile_aggregate_') && f.endsWith('.json'));
if(!files.length){ console.error('no profile_aggregate files found'); process.exit(1); }
files.sort();
...
for (const r of perReq) {
  const evs = r.events || [];
  for (const ev of evs) {
    const m = String(ev).match(/profile totalMs:(\d+)\s+gen:(\d+)\s+hits:(\d+)\s+misses:(\d+)/);
    if (m) {
      totalMatches++;
