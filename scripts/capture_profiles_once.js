...
async function runOnce({ durationMs = 8000 } = {}) {
  ...
  const serverArgs = [path.join('node_modules', 'http-server', 'bin', 'http-server'), '-p', String(port), '-c-1', '.'];
  const serverProc = cp.spawn(serverCmd, serverArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
  ...
  const earlyMs = process.env.EARLY_FALLBACK_MS ? Number(process.env.EARLY_FALLBACK_MS) : null;
  const planTimeoutMs = process.env.PLAN_TIMEOUT_MS ? Number(process.env.PLAN_TIMEOUT_MS) : null;
  const disableFallbacks = process.env.DISABLE_FALLBACKS ? true : null;
    const beamWidth = process.env.BEAM_WIDTH ? Number(process.env.BEAM_WIDTH) : null;
    const perNode = process.env.PER_NODE_LIMIT ? Number(process.env.PER_NODE_LIMIT) : null;
    const topK = process.env.TOP_K ? Number(process.env.TOP_K) : null;
  await page.evaluate((earlyMsArg, planTimeoutArg, disableFallbacksArg) => {
    try { window.__autoCapturedLogs = []; } catch (e) {}
    try { if (window.ai && typeof window.ai.clearLogs === 'function') window.ai.clearLogs(); } catch (e) {}
    try { if (window.ai && typeof window.ai.setDebugEnabled === 'function') window.ai.setDebugEnabled(true); } catch (e) {}
    try { if (earlyMsArg != null && window.ai) { try { window.ai.EARLY_FALLBACK_MS = earlyMsArg; } catch (e) {} } } catch (e) {}
    try { if (planTimeoutArg != null && window.ai) { try { window.ai.PLAN_TIMEOUT_MS = planTimeoutArg; } catch (e) {} } } catch (e) {}
    try { if (disableFallbacksArg != null && window.ai) { try { window.ai.DISABLE_FALLBACKS = !!disableFallbacksArg; } catch (e) {} } } catch (e) {}
    try { if (window.ai && typeof window.ai.setEnabled === 'function') window.ai.setEnabled(true); } catch (e) {}
  ...
  }, earlyMs, planTimeoutMs, disableFallbacks);
    // apply optional runtime tuning params
    await page.evaluate((bw, pnl, tk) => {
      try { if (typeof bw === 'number' && window.ai && typeof window.ai.setBeamWidthBase === 'function') window.ai.setBeamWidthBase(bw); } catch (e) {}
      try { if (typeof pnl === 'number' && window.ai && typeof window.ai.setPerNodeLimit === 'function') window.ai.setPerNodeLimit(pnl); } catch (e) {}
      try { if (typeof tk === 'number' && window.ai && typeof window.ai.setTopK === 'function') window.ai.setTopK(tk); } catch (e) {}
    }, beamWidth, perNode, topK);
  ...
  const outLogs = path.join(RESULTS_DIR, `capture_logs_${paramSuffix}_${timestamp}.json`);
  fs.writeFileSync(outLogs, JSON.stringify({ timestamp, logs }, null, 2), 'utf8');
  console.log('Saved logs to', outLogs);

  if (profiles) {
    const profPath = path.join(RESULTS_DIR, `worker_profiles_manual_${paramSuffix}_${timestamp}.json`);
...
runOnce({ durationMs: process.env.DURATION_MS ? Number(process.env.DURATION_MS) : 8000 }).catch(err => {
