...
const _bs = parseInt(
  process.env.BOOTSTRAP_REPS || process.argv[2] || "10000",
  10,
);
...
function ci95_for_mean(arr) {
  const m = mean(arr);
  if (m === null) return null;
  const n = arr.length || 0;
  if (n < 500) return bootstrap_ci_for_mean(arr, 2000);
  const sd = stddev(arr);
  const se = sd / Math.sqrt(n || 1);
  const margin = 1.96 * se;
  return {
    low: +(m - margin).toFixed(4),
    high: +(m + margin).toFixed(4),
    mean: +m.toFixed(4),
    std: +sd.toFixed(4),
    n,
  };
...
function gatherFiles() {
  ...
  const files = fs
    .readdirSync(dir)
    .filter((f) => /^timestamps_(fast|slow)_run\d+\.json$/i.test(f))
    .sort();
...
function main() {
  ...
  if (files.length === 0) {
    console.warn("no per-run timestamp files found");
    return;
  }
  const agg = {
    fast: { left: {}, right: {}, down: {} },
  ...
  for (const fp of files) {
    ...
    const variant = /fast/i.test(name)
      ? "fast"
      : /slow/i.test(name)
        ? "slow"
        : "other";
    try {
      ...
      console.warn("failed to parse", fp, e.message);
    }
  }
  ...
  for (const v of ["fast", "slow"]) {
    ...
    for (const r of runs) {
      ...
      if (la) {
        if (la.initialGap_ms !== null) leftInitials.push(la.initialGap_ms);
        if (Array.isArray(la.arrIntervals_ms))
          leftPooledARRs.push(...la.arrIntervals_ms);
        if (la.medianARR_ms !== null) leftMedianARRs.push(la.medianARR_ms);
      }
      const ra = r.analysis.right;
      if (ra) {
        if (ra.initialGap_ms !== null) rightInitials.push(ra.initialGap_ms);
        if (Array.isArray(ra.arrIntervals_ms))
          rightPooledARRs.push(...ra.arrIntervals_ms);
        if (ra.medianARR_ms !== null) rightMedianARRs.push(ra.medianARR_ms);
      }
      const d = r.analysis.down;
      if (d) {
        if (d.medianSoft_ms !== null) softMedians.push(d.medianSoft_ms);
        if (Array.isArray(d.softIntervals_ms))
          softPooled.push(...d.softIntervals_ms);
      }
    }
