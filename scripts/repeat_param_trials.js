...
console.log(`Running repeat param trials: ${combos.length} combos x ${TRIALS} trials, DURATION_MS=${DURATION_MS}`);
for (const c of combos) {
  for (let t = 0; t < TRIALS; t++) {
    ...
    const res = cp.spawnSync('node', [path.join('scripts','capture_profiles_once.js')], { env, stdio: 'inherit', cwd: process.cwd(), timeout: 180000 + DURATION_MS });
