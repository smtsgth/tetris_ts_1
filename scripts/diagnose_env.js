...
function safeWrite(path, data) {
  try {
    fs.writeFileSync(path, String(data || ""), "utf8");
  } catch (e) {}
}
...
try {
  ...
  safeWrite(".diagnostic_node.txt", "error:" + String(e));
}
...
try {
  ...
  safeWrite(".diagnostic_npm.txt", "error:" + String(e));
}
...
try {
  ...
  safeWrite(".diagnostic_node_path.txt", "error:" + String(e));
}
...
try {
  ...
  safeWrite(".diagnostic_npm_path.txt", "error:" + String(e));
}

try {
  const pwshVer = cp
    .execSync('pwsh -Command "$PSVersionTable.PSVersion.ToString()"', {
      timeout: 5000,
    })
    .toString();
  safeWrite(".diagnostic_pwsh_version.txt", pwshVer);
} catch (e) {
  safeWrite(".diagnostic_pwsh_version.txt", "error:" + String(e));
}
...
try {
  ...
  safeWrite(".diagnostic_env.txt", "error:" + String(e));
}
