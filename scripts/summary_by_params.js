...
const files = fs
  .readdirSync(REC_DIR)
  .filter((f) => f.startsWith("worker_profiles") && f.endsWith(".json"));
if (!files.length) {
...
for (const f of files) {
  try {
    ...
    stats.params = {
      beamWidth: bwMatch ? Number(bwMatch[1]) : null,
      perNodeLimit: pnlMatch ? Number(pnlMatch[1]) : null,
...
const groupList = Object.values(groups).map((g) => {
  const avgTotalTimeMs = g.profilesWithStats
    ? g.totalTimeMsSum / g.profilesWithStats
    : null;
  const overallHitRate =
    g.placementHitsSum + g.placementMissesSum
      ? g.placementHitsSum / (g.placementHitsSum + g.placementMissesSum)
      : null;
  const avgPlacementGenerated = g.profilesWithStats
    ? g.placementGeneratedSum / g.profilesWithStats
    : null;
  return Object.assign({}, g, {
    avgTotalTimeMs,
    overallHitRate,
    avgPlacementGenerated,
  });
});
groupList.sort((a, b) => {
  ...
  const aT =
    a.avgTotalTimeMs === null ? Number.POSITIVE_INFINITY : a.avgTotalTimeMs;
  const bT =
    b.avgTotalTimeMs === null ? Number.POSITIVE_INFINITY : b.avgTotalTimeMs;
  return aT - bT;
});
const out = { generatedAt: Date.now(), totals, groups: groupList, perFile };
const outPath = path.join(
  REC_DIR,
  `profile_summary_by_params_${Date.now()}.json`,
);
...
for (let i = 0; i < Math.min(5, groupList.length); i++) {
  const g = groupList[i];
  console.log(
    `${i + 1}. ${g.key}  hitRate=${g.overallHitRate !== null ? g.overallHitRate.toFixed(4) : "n/a"} avgTotalTimeMs=${g.avgTotalTimeMs !== null ? g.avgTotalTimeMs.toFixed(2) : "n/a"} runs=${g.runs} files=${g.files.length}`,
  );
}
process.exit(0);
