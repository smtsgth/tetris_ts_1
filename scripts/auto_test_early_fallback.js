...
const GLOBAL_RUN_EVENTS_PATH = path.join(
  RESULTS_DIR,
  `auto_test_run_events_${Date.now()}.json`,
);
...
function recordEvent(ev) {
  try {
    ...
    fs.writeFileSync(
      GLOBAL_RUN_EVENTS_PATH,
      JSON.stringify(__auto_test_events, null, 2),
      "utf8",
    );
  } catch (e) {}
}
...
function writeExitFile(code) {
  try {
    fs.writeFileSync(
      path.join(RESULTS_DIR, "cli_exitcode_run.txt"),
      String(code),
      "utf8",
    );
  } catch (e) {}
...
function handleFatal(kind, err) {
  ...
  try {
    fs.appendFileSync(path.join(RESULTS_DIR, `${kind}_${ts}.log`), msg + "\n");
  } catch (e) {}
  try {
    fs.appendFileSync(
      path.join(RESULTS_DIR, `run_fatal_${ts}.log`),
      msg + "\n",
    );
  } catch (e) {}
  try {
    __auto_test_events.push(
      Object.assign({ timestamp: ts }, { type: "fatal", kind, error: msg }),
    );
    fs.writeFileSync(
      GLOBAL_RUN_EVENTS_PATH,
      JSON.stringify(__auto_test_events, null, 2),
...
process.on("uncaughtException", (err) => {
  try {
    fs.appendFileSync(
      path.join(RESULTS_DIR, `run_uncaught_exception_${Date.now()}.log`),
...
process.on("unhandledRejection", (err) => {
  try {
    fs.appendFileSync(
      path.join(RESULTS_DIR, `run_unhandled_rejection_${Date.now()}.log`),
      err && err.stack ? err.stack : String(err),
    );
  } catch (e) {}
  recordEvent({
...
try {
  ...
  try {
    fs.appendFileSync(
      path.join(RESULTS_DIR, `run_start_error_${Date.now()}.log`),
...
const TEST_DURATION_MS = process.env.TEST_DURATION_MS
  ? Number(process.env.TEST_DURATION_MS)
  : 15 * 1000; // ms per test (can override via env)
...
function replaceEarly(ms) {
  ...
  if (!re.test(code))
    throw new Error("EARLY_FALLBACK_MS pattern not found in " + AI_SRC);
...
async function run() {
  ...
  for (const ms of TESTS) {
    ...
    const serverLogPath = path.join(
      RESULTS_DIR,
      `server_${portToUse}_${Date.now()}.log`,
    );
    const serverErrPath = path.join(
      RESULTS_DIR,
      `server_${portToUse}_${Date.now()}.err`,
    );
    if (serverProc.stdout)
      serverProc.stdout.on("data", (d) => {
        process.stdout.write(`[server:${portToUse}] ${d}`);
        try {
          fs.appendFileSync(serverLogPath, d);
        } catch (e) {}
      });
    if (serverProc.stderr)
      serverProc.stderr.on("data", (d) => {
        ...
        try {
          fs.appendFileSync(serverErrPath, d);
        } catch (e) {}
      });
    serverProc.on("exit", (code, sig) => {
      ...
      if (typeof code === "number" && code !== 0) {
        try {
          handleFatal(
            "server-exit",
            new Error(`server exited unexpectedly code=${code} signal=${sig}`),
          );
        } catch (e) {}
      }
    });
    serverProc.on("error", (err) => {
      ...
      recordEvent({
        type: "server-proc-error",
        port: portToUse,
        error: err && err.stack ? err.stack : String(err),
      });
      try {
        handleFatal("server-proc-error", err);
      } catch (e) {}
    });
    ...
    try {
      ...
      try {
        fs.appendFileSync(
          serverErrPath,
          `Server did not start: ${e && e.stack ? e.stack : e}\n`,
        );
      } catch (er) {}
      recordEvent({
    ...
    try {
      ...
      const outPath = path.join(
        RESULTS_DIR,
        `auto_test_early_fallback_${ms}_${Date.now()}.json`,
      );
      ...
      try {
        ...
        try {
          ...
          try {
            fs.appendFileSync(
              path.join(
                RESULTS_DIR,
                `profile_capture_error_${ms}_${Date.now()}.log`,
              ),
              e && e.stack ? e.stack : String(e),
            );
          } catch (er) {}
        ...
        if (profiles) {
          try {
            const profPath = path.join(
              RESULTS_DIR,
              `worker_profiles_${ms}_${Date.now()}.json`,
            );
            fs.writeFileSync(
              profPath,
            ...
            try {
              fs.appendFileSync(
                path.join(
                  RESULTS_DIR,
                  `profile_write_error_${ms}_${Date.now()}.log`,
                ),
                e && e.stack ? e.stack : String(e),
              );
            } catch (er) {}
        ...
        try {
          fs.appendFileSync(
            path.join(
              RESULTS_DIR,
              `profile_capture_outer_error_${ms}_${Date.now()}.log`,
            ),
            e && e.stack ? e.stack : String(e),
          );
        } catch (er) {}
      ...
      try {
        fs.appendFileSync(
          path.join(RESULTS_DIR, `run_error_${ms}_${Date.now()}.log`),
          e && e.stack ? e.stack : String(e),
        );
      } catch (er) {}
      recordEvent({
    ...
    try {
      serverProc.kill();
      recordEvent({ type: "server-killed", port: portToUse });
    } catch (e) {
      recordEvent({
        type: "server-kill-failed",
        port: portToUse,
    ...
    if (!process.env.SKIP_AUTOTEST_RESTORE) {
      if (fs.existsSync(BACKUP)) {
        ...
        try {
          ...
          console.error("Rebuild of original failed:", e);
        }
      }
    } else {
      console.log(
        "Skipping restore of ai.ts because SKIP_AUTOTEST_RESTORE is set",
      );
    }
  } catch (e) {
  ...
  const summaryPath = path.join(
    RESULTS_DIR,
    `auto_test_summary_${Date.now()}.json`,
  );
...
run().catch((err) => {
  ...
  try {
    fs.appendFileSync(
      path.join(RESULTS_DIR, `run_fatal_${Date.now()}.log`),
      err && err.stack ? err.stack : String(err),
    );
  } catch (e) {}
  recordEvent({
...
process.on("exit", (code) => {
  try {
    recordEvent({ type: "process-exit", code: code });
  } catch (e) {}
