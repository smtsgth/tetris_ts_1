...
async function runOnce(durationMs = 5000) {
  ...
  try {
    ...
    page.on("console", async (msg) => {
      try {
        ...
        for (let i = 0; i < args.length; i++) {
          const v = await args[i].jsonValue();
          console.log("PAGE_CONSOLE:", v);
        }
      } catch (e) {
        console.log("PAGE_CONSOLE_ERR", e);
      }
    });
    ...
    const logs = await page.evaluate(() => {
      try {
        return window.ai && typeof window.ai.getLogs === "function"
          ? window.ai.getLogs()
          : [];
      } catch (e) {
        return [];
