...
function parseFile(filePath){
  ...
  const anomalyRegex = /\[([^\]]+)\]\s+\(console\)\s+\[warning\]\s+Lock anomaly detected\s+{ts:\s*([0-9]+),\s*type:\s*([^,}]+),\s*piece:\s*([A-Z]+),\s*beforeFilled:\s*([0-9]+),\s*afterFilled:\s*([0-9]+)}/i;
  const snapshotRegex = /Stats:\s*total=(\d+),\s*anomalies=(\d+),\s*last=(\d+)/i;
  ...
  for(const line of lines){
    ...
    if(ms){
      snapshot = { total: Number(ms[1]), anomalies: Number(ms[2]), last: Number(ms[3]) };
    }
  }

  const earliest = isoTimes.length ? Math.min(...isoTimes) : (anomalies.length ? anomalies[0].ts : null);
  const latest = isoTimes.length ? Math.max(...isoTimes) : (anomalies.length ? anomalies[anomalies.length-1].ts : null);
  ...
  const pieceCounts = {};
  for(const a of anomalies) pieceCounts[a.piece] = (pieceCounts[a.piece]||0) + 1;

  const totalEvents = snapshot ? snapshot.total : null;
  const snapshotAnom = snapshot ? snapshot.anomalies : null;

  const eventsPerSec = (durationSec && totalEvents) ? (totalEvents/durationSec) : null;
  const anomaliesPerSec = (durationSec && snapshotAnom) ? (snapshotAnom/durationSec) : null;
  const anomalyRatio = (totalEvents && snapshotAnom) ? (snapshotAnom/totalEvents*100) : null;

  const topPieces = Object.entries(pieceCounts).sort((a,b)=>b[1]-a[1]).slice(0,5);
...
try{ fs.writeFileSync('scripts/session_analysis.json', JSON.stringify(out, null, 2)); }catch(e){/* ignore */}
