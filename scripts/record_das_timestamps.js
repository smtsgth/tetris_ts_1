...
(async () => {
  ...
  const initialReady = await page.evaluate(() => document.readyState).catch(() => 'error');
  ...
  try {
    await page.waitForFunction(() => !!window.game && typeof window.game.getState === 'function', { timeout: 60000 });
    ...
    } catch (e) { console.error('failed to enumerate window keys', e); }
    throw err;
  }
  ...
  for (const v of variants) {
    ...
    fs.writeFileSync(`recordings/timestamps_${v.name}.json`, JSON.stringify(results, null, 2));
