...
const prefix = "stability_p4_t2_b1_long1000_run";
...
fs.writeFileSync(
  path.join(dir, "stability_p4_t2_b1_long1000_summary.json"),
  JSON.stringify(summary, null, 2),
);
let md = `# stability_p4_t2_b1_long1000 summary\n\nRuns: ${n}\nMean: ${mean}\nStd: ${std}\nMin: ${min}\nMax: ${max}\nMedian: ${median}\n\nFiles:\n`;
md += files.map((f) => `- ${f}`).join("\n");
fs.writeFileSync(path.join(dir, "stability_p4_t2_b1_long1000_summary.md"), md);
let csv =
  "filename,meanOfMeans\n" + files.map((f, i) => `${f},${vals[i]}`).join("\n");
fs.writeFileSync(
  path.join(dir, "stability_p4_t2_b1_long1000_summary.csv"),
  csv,
);
