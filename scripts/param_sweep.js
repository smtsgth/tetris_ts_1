...
(async () => {
    ...
    for (const hm of holdMsList) {
      for (const mi of maxIntermediateList) {
        const params = {
          DEBOUNCE_MS: dm,
          DEBOUNCE_HOLD_MS: hm,
          MAX_INTERMEDIATE_APPLIES_PER_REQ: mi,
        };
        ...
        try {
          ...
          const skipDup = logs.filter(
            (l) =>
              typeof l === "string" && l.indexOf("skipping duplicate") !== -1,
          ).length;
  ...
  const outPath = path.join(
    recordingsDir,
    `param_sweep_summary_${Date.now()}.json`,
  );
  fs.writeFileSync(
    outPath,
    JSON.stringify({ timestamp: Date.now(), duration, results }, null, 2),
    "utf8",
  );
  console.log("Sweep complete. Summary saved to", outPath);
  console.table(
    results.map((r) => ({
      dm: r.params.DEBOUNCE_MS,
      hm: r.params.DEBOUNCE_HOLD_MS,
      mi: r.params.MAX_INTERMEDIATE_APPLIES_PER_REQ,
      apply: r.applyCount,
      hold: r.holdCount,
      skipped: r.skipDup,
    })),
