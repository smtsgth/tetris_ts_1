...
async function runOnce(durationMs = 5000) {
  ...
  await new Promise((resolve, reject) => {
    server.listen(portToUse, '127.0.0.1', (err) => err ? reject(err) : resolve());
  });
  ...
  } catch (e) { console.warn('invalid ai params, ignoring'); aiParams = null; }
  ...
  try {
        ...
        if (params && window.ai) {
          try {
            Object.keys(params).forEach(k => {
              try { (window.ai)[k] = params[k]; } catch (e) {}
            });
          } catch (e) {}
        }
      } catch (e) {}
    ...
    const outPath = path.join(__dirname, '..', 'recordings', `capture_one_${Date.now()}.json`);
