...
function processLogs(lines, file, ctx) {
  ...
  lines.forEach((ln, idx) => {
    ...
    if ((m = ln.match(reWorkerRoundtrip)))
      ctx.workerRoundtrip.push(Number(m[1]));
...
function main() {
  ...
  const allFiles = listFiles(recordingsDir).filter((f) =>
    /\.json$|\.txt$|\.log$/i.test(f),
  );
  ...
  fs.writeFileSync(
    path.join(outDir, "analysis_summary.json"),
    JSON.stringify(
      {
        summary,
        samples: {
          startToFirst: ctx.startToFirst.length,
          workerRoundtrip: ctx.workerRoundtrip.length,
          profileTotal: ctx.profileTotal.length,
        },
      },
      null,
      2,
  ...
  writeCSV(
    path.join(outDir, "start_to_first.csv"),
    ["ms"],
    ctx.startToFirst.map((v) => [v]),
  );
  writeCSV(
    path.join(outDir, "worker_roundtrip.csv"),
    ["ms"],
    ctx.workerRoundtrip.map((v) => [v]),
  );
  writeCSV(
    path.join(outDir, "profile_total.csv"),
    ["ms"],
    ctx.profileTotal.map((v) => [v]),
  );
  ...
  writeCSV(
    path.join(outDir, "lock_anomalies.csv"),
    [
      "ts",
      "type",
      "piece",
  ...
  writeCSV(
    path.join(outDir, "fallback_summary.csv"),
    ["metric", "value"],
    [
      ["reqStarts", ctx.reqStarts],
      ["earlyFallbackCount", ctx.earlyFallbackCount],
