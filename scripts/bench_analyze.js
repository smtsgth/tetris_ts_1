...
const infile = process.argv[2] ? path.resolve(process.argv[2]) : path.join(process.cwd(), 'bench_results', 'combo_bench_full_2026-05-24.json');
...
let raw;
try { raw = fs.readFileSync(infile, 'utf8'); } catch (e) { console.error('Read error', e.message); process.exit(2); }
...
function percentile(arr, p){ if (!arr.length) return 0; const s = arr.slice().sort((a,b)=>a-b); const idx = Math.max(0, Math.min(s.length-1, Math.ceil(p * s.length)-1)); return s[idx]; }

// Normalize: if mean/median/p90 not present, compute from sampleTimes or timesSample
results.forEach(r => {
  const samples = Array.isArray(r.sampleTimes) ? r.sampleTimes : (Array.isArray(r.timesSample) ? r.timesSample : (Array.isArray(r.times) ? r.times : []));
  if (samples && samples.length) {
    r.meanMs = Number((samples.reduce((a,b)=>a+b,0)/samples.length).toFixed(3));
    const sorted = samples.slice().sort((a,b)=>a-b);
    r.medianMs = sorted.length % 2 ? sorted[(sorted.length-1)/2] : ((sorted[sorted.length/2 -1] + sorted[sorted.length/2]) / 2);
    const p90Idx = Math.max(0, Math.min(sorted.length-1, Math.ceil(0.9 * sorted.length)-1));
...
const medianOfMeans = (function(){ if (!means.length) return 0; const s = means.slice().sort((a,b)=>a-b); const mid = Math.floor(s.length/2); return (s.length%2) ? s[mid] : (s[mid-1]+s[mid])/2; })();
const p90OfMeans = percentile(means, 0.9);

const experimentsWithTimeouts = results.filter(r => Number(r.timeoutCount || 0) > 0).length;
...
const poolSummary = {};
Object.keys(byPool).forEach(k => { poolSummary[k] = { count: byPool[k].count, avgMeanMs: byPool[k].sumMean / byPool[k].count }; });
...
const md = [
  ...
  `## Pool ごとの平均 meanMs`,
  ``,
  ...Object.keys(poolSummary).map(k => `- pool ${k}: ${poolSummary[k].count} 件, 平均 meanMs = ${poolSummary[k].avgMeanMs.toFixed(3)} ms`),
  ``,
  `---`,
