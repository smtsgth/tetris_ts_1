  ...
  try {
    ...
    function isTopLevel(node) {
      ...
      while (p && p.kind !== ts.SyntaxKind.SourceFile) {
        if (ts.isFunctionLike(p) || ts.isClassDeclaration(p) || ts.isMethodDeclaration(p) || p.kind === ts.SyntaxKind.Constructor) return false;
        p = p.parent;
      }
      return true;
    }
      ...
      if (ts.isPropertyAccessExpression(node) || ts.isElementAccessExpression(node)) {
        ...
        if (ts.isIdentifier(left) && left.text === 'document') {
          if (isTopLevel(node)) {
            ...
            let allowed = false;
            if (ts.isPropertyAccessExpression(node) && node.name && node.name.text === 'readyState') {
              allowed = true;
            }
            if (!allowed && ts.isPropertyAccessExpression(node) && node.name && node.name.text === 'addEventListener') {
              const par = node.parent;
              if (par && ts.isCallExpression(par) && par.arguments && par.arguments.length > 0) {
                const arg0 = par.arguments[0];
                if ((ts.isStringLiteral(arg0) || ts.isNoSubstitutionTemplateLiteral(arg0)) && arg0.text === 'DOMContentLoaded') {
                  allowed = true;
                }
              }
            }
            if (!allowed) {
              ...
              findings.push({ file, line: line + 1, column: character + 1, text: node.getText(sf) });
            }
          }
        }
      }
...
if (findings.length > 0) {
  ...
  for (const f of findings) {
    console.log(`- ${path.relative(root, f.file)}:${f.line}:${f.column}  ->  ${f.text}`);
  }
